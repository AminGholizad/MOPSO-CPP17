...
namespace mopso {
  ...
  std::pair<Repository<N,O>,std::array<Particle<N,O>,S>> mopso(const vars<N>& lower_bound,const vars<N>& upper_bound,const Problem<N,O>& problem,const size_t max_iter=1000,size_t Rep_size=100,const std::array<double,2>& c={0.2,0.2},const std::array<double,2>& iw={0.1,0.01},const double mu=0.1){
    ...
    Repository<N,O> rep(swarm,Rep_size,7,0.1,2,2);
